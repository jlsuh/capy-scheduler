...
void test_un_proceso_retiene_una_instancia_del_semaforo_en_caso_de_no_bloquearse(void) {
    ...
    CU_ASSERT_TRUE(retiene_instancias_del_semaforo(pcb1, 1, sem1));
}
...
void test_un_proceso_retiene_dos_instancias_de_un_semaforo(void) {
    ...
    CU_ASSERT_TRUE(retiene_instancias_del_semaforo(pcb5, 2, sem5));
}
...
static bool eliminar_celdas_nulas_consecutivas(t_list* firstToIterateList, t_dimention_type firstToIterateType, t_list* pivotList) {
    /* Eliminar:
...
void test_es_posible_eliminar_columnas_y_filas_nulas_1(void) {
    ...
    t_list* pcbsDeadlock = list_filter(pcbsBlocked, espera_algun_semaforo);

    CU_ASSERT_EQUAL(list_size(pcbsDeadlock), 5);
    CU_ASSERT_EQUAL(list_size(semsDeadlock), 7);

    reducir_matrices_de_deteccion(pcbsDeadlock, semsDeadlock);
...
static void detectar_y_recuperarse_del_deadlock(t_list* pcbsBlocked, t_cola_recursos* semaforosDelSistema) {
    t_list* pcbsDeadlock = list_filter(pcbsBlocked, espera_algun_semaforo); /* TODO: Tomar mutex de pcbsBlocked & pcbsSusBlocked */
    t_list* semsDeadlock = list_create();
    ...
    bool existeDeadlock = reducir_matrices_de_deteccion(pcbsDeadlock, semsDeadlock);
    ...
    while(existeDeadlock) {
        ...
        print_awaiting_resources_matrix(pcbsDeadlock, semsDeadlock);
        print_alloced_resources_matrix(pcbsDeadlock, semsDeadlock);
        existeDeadlock = reducir_matrices_de_deteccion(pcbsDeadlock, semsDeadlock);
        print_awaiting_resources_matrix(pcbsDeadlock, semsDeadlock);
        print_alloced_resources_matrix(pcbsDeadlock, semsDeadlock);
    }
...
void test_deteccion_y_recuperacion_del_deadlock_1(void) {
    ...
    recuperarse_del_deadlock(pcbsDeadlock, semsDeadlock, colaSems->listaRecursos, pcbsBlocked);
